...
function Favoritos() {
    ...
    return ( 
        <div className="max-w-7xl mx-auto px-8">
            ...
            {favoritos.length === 0 ? (
                <div className="text-center mt-8 flex flex-col items-center gap-4 pt-24">
                    <span className="text-4xl">🎬</span>
                    <p className="text-gray-500">No tienes películas favoritas aún</p>
                    <Link to="/" className="text-blue-500 hover:underline text-sm">Explora películas populares</Link>
                </div>
                ...
                <div className='grid grid-cols-5 p-8 gap-x-8 gap-y-8'>
                    {favoritos.map(pelicula => (
                        <div key={pelicula.id} className="border border-gray-300 min-h-90 rounded-lg shadow-sm flex flex-col justify-between">
                            ...
                            <button
                                onClick={() => eliminarFavorito(pelicula.id)}
